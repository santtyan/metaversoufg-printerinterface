...
"""
K1 Max Controller - Implementa as 5 funcoes
1. is_printing() - checar se esta imprimindo
2. is_ready() - checar se esta livre
3. set_material() - checa o material
4. set_temperature() - checa a temperatura
5. send_print_job() - enviar objeto para impressao
"""
...
class K1MaxController:
    ...
    def is_printing(self):
        """1. Checar se esta imprimindo"""
        return self.monitor.is_printing()
    
    def is_ready(self):
        """2. Checar se esta livre e pronta"""
    ...
    def set_temperature(self, nozzle, bed):
        ...
        if not (180 <= nozzle <= 300):
            raise ValueError(f"Temperatura nozzle invalida: {nozzle}")
        if not (0 <= bed <= 110):
            raise ValueError(f"Temperatura bed invalida: {bed}")
        
        print(f"Temperatura: {nozzle}C nozzle, {bed}C bed (configurado no preset)")
        return True
    
    def send_print_job(self, file_path):
        """5. Enviar objeto para impressao"""
        file = Path(file_path)
        if not file.exists():
        ...
        print(f"Enviando '{file.name}' para impressao...")
